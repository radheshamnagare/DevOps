...
# Function 3 & 4
def aroon(data,userTimeperiod,userValueType,userInputValue):
    expectation_res = ""
    try:
        aroondown, aroonup = talib.AROON(data['High'],data['Low'], timeperiod=userTimeperiod)
        data['AroonUP'] = aroonup
        data['AroonDOWN'] = aroondown
        max_of_aroonUP = data['AroonUP'].max()
        max_of_aroonDOWN = data['AroonDOWN'].max()

        if(userValueType == 'AroonUP'):
            if(max_of_aroonUP < userInputValue):
                expectation_res = ("User input is Higher")
            elif(max_of_aroonUP > userInputValue):
                expectation_res = ("User input is Lower")
            else:
                expectation_res = ("User input is Equal")

        else:
            if(max_of_aroonDOWN < userInputValue):
                expectation_res = ("User input is Higher")
            elif(max_of_aroonDOWN > userInputValue):
                expectation_res = ("User input is Lower")
            else:
                expectation_res = ("User input is Equal")
...
def handle(req):
    ...
    try:
        ...
        if(True):
            ...
            res_data,expectation_res = aroon(data,userTimeperiod,userValueType,userInputValue)
